...
select movie_id, count(genre) as count_of_genre from genre 
group by movie_id)

SELECT 
    COUNT(count_of_genre) AS single_genre_movie_count
FROM
    movie_genre_summary
WHERE
    count_of_genre = 1;
...
FROM
    movie AS m
        LEFT JOIN
    ratings AS r ON m.id = r.movie_id limit 10;
...
select production_company, movie_count, row_number() over(order by movie_count desc) as production_company_rank
from top_prod limit 2;
...
-- Type your code below:
with summary_of_actors as (select n.name, n.id, median_rating, category
from names n 
...
SELECT 
    name AS actor_name, COUNT(id) AS movie_count
FROM
    summary_of_actors
WHERE
    median_rating >= 8
...
-- Type your code below:
with actress_ratings as (select n.name, r.total_votes, r.movie_id, avg_rating, rm.category,  m.country, m.languages
from names n 
join role_mapping rm on n.id = rm.name_id
join movie m on rm.movie_id = m.id
join ratings r on rm.movie_id = r.movie_id)
...
FROM 
    actress_ratings
WHERE 
    country = 'india' AND languages = 'hindi' AND category = 'actress'
GROUP BY 
    name
HAVING 
    COUNT(movie_id) >= 3
